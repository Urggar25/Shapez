const { existsSync, readdirSync, readFileSync, writeFileSync, copyFileSync, unlinkSync } = require("fs");
// @ts-ignore
...
function gulptasksImageResources($, gulp, buildFolder) {
    ...
    gulp.task("imgres.buildAtlas", cb => {
        ...
        try {
            ...
            // Run packer without settings file (newer jar versions changed JSON field names)
            execute(`java -jar runnable-texturepacker.jar ${source} ${dest} atlas0`);

            // The packer generates atlas0.atlas — duplicate it for each quality level
            // so atlasToJson can find the expected _lq/_mq/_hq suffixed files
            const atlasDir = path.resolve("../res_built/atlas");
            const atlasFiles = readdirSync(atlasDir).filter(f => f.endsWith(".atlas"));
            for (const f of atlasFiles) {
                const base = f.replace(/\.atlas$/, "");
                const content = readFileSync(path.join(atlasDir, f), "utf-8");
                for (const suffix of ["_lq", "_mq", "_hq"]) {
                    writeFileSync(path.join(atlasDir, base + suffix + ".atlas"), content, "utf-8");
                }
                unlinkSync(path.join(atlasDir, f));
            }
        } catch(e) {
            console.warn("Building atlas failed. Java not found / unsupported version?", e.message);
        }
        cb();
    });
